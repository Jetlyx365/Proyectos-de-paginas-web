...
// Seleccionar con que puedo buscar usuarios
if ($search != '') {
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $table_html .= '<tr>
                            ...
                            <td>' . $row["DNI"] . '</td>
                            <td>' . $row["Telefono"] . '</td>
                            <td><a href="../metadatos_admin/actualizar.php?id=' . $row["id"] . '"><button id="edit_but">Actualizar</button></a></td>
                            <td><a href="?id=' . $row["id"] . '&page=' . $page . '&search=' . $search . '" onclick="return confirm(\'¿Estás seguro que deseas eliminar el registro?\');"><button id="elim_but">Eliminar</button></a></td>
                        </tr>';
    }
} else {
